...
- 对应任务：犀牛鸟开源实战任务 1，开放式场景 AI 应用与评判标准设计
- 文档版本：V2.1，项目实施方案
- 编制日期：2026-08-22
- 项目交付日期：2026-09-11
- 项目性质：个人开源项目，非腾讯官方项目
- 当前状态：方案与前置调研完成，功能和实验结果待实现验证
...
| 自建样本与难例 | 15 篇开放许可论文、30 份三档输出和 16 个攻击样例 | manifest、构造日志和已知错误标签 |
...
论文按领域与解析难度交叉选择。每篇保存 `paper_id`、标题、DOI/PMCID/URL、许可名称和 URL、核验日期、文件 hash 及允许公开的材料类型。公开仓库不提交许可不明的全文，只提交许可允许的片段、标签、下载脚本或公开 URL。

数据在实验前写入冻结的 manifest：

- 开发集 5 篇：调整解析参数、prompt、rubric、权重和门槛。
- 保留集 10 篇：冻结后运行，用于最终结果和失败归因。
...
成对比较交换左右顺序，记录排序是否改变。目标为百分制总分平均标准差不超过 5 分，核心维度等级一致率不低于 80%。ICC 可作为附加描述，不作为小样本硬门槛。
...
| Hy3 生成与裁判存在自偏好 | 高 | 规则优先、已知错误标签、盲化、顺序交换、重复运行和独立上下文 |
| 开发集污染保留集 | 高 | 5/10 划分、冻结 manifest、校准与最终三档样例分离 |
...
| 时间 | 展示内容 |
|---|---|
| 0至15秒 | 上传开放许可 PDF，展示解析状态和当前文档 |
| 15至40秒 | 展示研究问题、方法、结果、限制和通俗解释 |
| 40至65秒 | 点击正确主张和范围夸大主张，跳转并高亮页码证据 |
| 65至95秒 | 与 Hy3 讨论句子修改，预览补丁并接受，执行局部复核 |
| 95至110秒 | 展示八维分数、原始指标、双门槛状态和版本回退 |
| 110至120秒 | 展示判别力、稳定性、攻击检出和失败案例总览 |
...
1. 用户提供的《犀牛鸟开源-实战任务-混元大语言模型项目》PDF，4页，核验日期2026-08-21。
