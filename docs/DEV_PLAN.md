...
- **数据范围**：本阶段采用现有 10 篇，开发 5 篇、保留 5 篇。原提案的 15 篇及领域、版式配额不再作为本阶段承诺；覆盖和能力限制须按实际清单披露，不能据此声称完成原 15 篇验证或复杂 PDF/OCR 验证。保留集的已使用状态按方案书第 7.5 节解释。
- **稳定性口径**：核心维度为事实一致性、引用准确性、引用完整性、方法与范围、结论与限制、术语准确性、读者适配与结构、风险与合规，共 8 项，与 `models.py` 的 `CORE_DIMENSION_GATES` 一致。正式计划逐个输出核验三次重复；同一输出同一维度的三次等级完全相同才计为一致，完整覆盖时分母为 12 × 8 = 96。保留现有每输出三次总分的总体标准差及其算术平均口径，平均标准差不超过 5，维度一致率不低于 80%；不改为 6 项，不改变权重或分档。
- **左右顺序交换**：当前先逐文档审计评分，再由代码比较分数形成成对排序，没有同时呈现给模型的左右候选；左右候选交换实验不适用于此方法，须披露未验证成对裁判的位置偏差。九次 dev-02 上下文诊断仅作开发诊断，不能替代左右交换实验或正式 stability；本次不新增模式、调用或实验。
- **证据适用性**：旧 final 已执行且结果已查看。旧原始记录保留为历史执行、失败和未知的证据；受指标实现或定义修订影响的旧聚合指标仅作历史/探索证据。未受影响的指标须核对输入、版本、定义和覆盖后明确批准复用。未来正式结论须对应事先批准的方法、有效冻结、完整覆盖及可重建结果；已查看数据上的重分析或复验只能支持获批的固定样本结论，不得称为全新盲测或独立泛化证据。

该轮口径确认不授权新的数据、Live、费用、冻结、提交或旧结果改写。`S7-CONTRACT-01` 此后按下述 A—F 单独定案并通过离线验收；`S7-P1-01` 稳定性逐输出覆盖和 `S7-P2-01` 论文计数返修也已通过独立验收。数值达标、覆盖完整和版本有效分别核验，阶段 7 与阶段 8 入口继续 HOLD。

本原子任务只做文档差异、残留冲突表述、`git diff --check` 及前后产物哈希核验，不运行测试或评测；下列测试命令仍适用于对应的后续代码任务或整体验收。

**已批准原子任务：定案状态与固定样本复验方案回填（2026-09-07）。** 唯一目标是将已批准的 A—F、证据适用性、复验顺序和授权边界同步到 `docs/DEV_PLAN.md` 与 `docs/paperlens_project_proposal.md`。本任务仅修改这两个文件；不改变输入输出字段、错误码、Prompt、Schema、样本、阈值或运行行为，保留全部前序差异和真实产物。只做文档差异、旧待定表述、`git diff --check` 及保护哈希核验，不运行测试；不暂存、提交、建立隔离副本、冻结或执行付费调用。

`S7-CONTRACT-01` 的 A—F 固定契约已批准，离线实现已通过独立验收。以下描述既有版本；文档级表达审计的新契约及其生效边界见本节后文，不能把新设计当作已经实施：

- **A**：目标主张的既有确定性与语义信号不变，不一概忽略目标 minor 异常。
- **B**：非目标语义只由 major/critical 保留命中；none/minor 的 relation、scope、terminology 异常不能单独命中，也不屏蔽独立有效的目标或维度信号。
- **C**：维度回退仅在没有目标 claim 时适用，保留 `limitation_deletion → conclusion_limitations`、`terminology_stuffing → terminology`、`rubric_prompt_injection → risk_compliance`、`length_padding → reader_adaptation` 四类映射及 `points < 4`。
- **D**：攻击和干净配对使用同一规则；不按身份、质量标签或历史结果特判，不改变模型输入。
- **E**：非目标严重问题命中只代表保守告警，不证明攻击因果归属。
- **F**：这是指标定义修订；不改历史 final，不重建或改判历史报告，攻击检出至少 13/16、干净误报不超过 1/16 等既有数值目标不变。后续证据只按批准的适用范围解释。

**版本衔接。** 本方案记录的基线为 `master / 485cad69ad9f529413bc8bd5b216b4edbb907dcc` 加已验收工作树差异，运行代码指纹为 `workspace-content-e2a86459e4cc5b73f271e1b0db3c679e9b154add0a81f16bea3f1feae29aa0eb`。它与旧冻结的代码指纹不同，旧冻结与当前 manifest 相符不能消除代码漂移。沿用 `audit-v5`、`deep-audit-result-v2` 和 `paperlens-stage7-freeze-v2`；将批准契约、Git 提交及内容指纹关联记录，不新增 Schema 字段。提交和冻结须分别授权，并在执行前重新核对精确基线。

**已批准的方法方案。** 在现有 10 篇（开发 5／保留 5）固定材料上安排一次有停止条件的复验，不增加样本，不将旧结果复制到新批次，不将已查看的保留集称为全新盲测或独立泛化证据。历史执行、失败和未知继续保留；历史质量指标经适用性核验后只能在明确批准范围内引用，不能补齐新批次。旧攻击、修订 JSONL 不含完整重算材料，不能从旧聚合数字推定新口径结果；九次上下文诊断不能替代正式 stability。

| 顺序 | 模式及固定结果槽 | 逻辑请求计划 | 每步最多 3 次尝试时的供应商尝试数上界 |
|---|---|---:|---:|
| 1 | calibrate：开发 5 篇 × 3 档，共 15 槽 | 15 | 45 |
| 2 | final：15 质量 + 16 攻击 + 16 干净配对 + 5 修订，共 52 槽 | 67 | 201 |
| 3 | stability：12 固定输出 × 3 次，共 36 槽 | 36 | 108 |
| 合计 | 103 个结果槽 | 118 | 354 |

修订每槽含 4 个逻辑步骤；上述是完整流程计划，提前失败可能减少实际调用，不能据此删去失败槽。尝试数上界以每步最多 2 次重试为条件，不是账单硬上限。费用授权前须重新核验有效非敏感配置、现价、Token 假设、预算及可执行的限制，不沿用历史价格作为现价。

**顺序、恢复和停止条件。** 开发集复验满足适用的质量门槛及完整覆盖后，才申请冻结和 final。final 有硬门槛失败或覆盖缺口时保持 HOLD，不自动继续花费于 stability。每次收费前在对应输出锁内核对完成、pending、中断和未知数量，列出新增逻辑请求数和供应商尝试上界；已完成键跳过，未完成 pending 按既有规则保留 `RUN_INTERRUPTED`，不自动重试。保留每条失败及未知，不改标签、阈值，不反复运行取优。正式实验不达标时报告能力限制；Stage 8 仍要求全部正式条件及最终基线的后端、前端回归证据。

**产物隔离与单独授权。** 拟采用 `D:\Hy3-stage7-contract01-e2a86459` 作为隔离工作副本，创建前另行授权并核对路径。现有 CLI 固定读取工作副本内的 `reports/stage7_frozen_config.json`；新副本在获批后生成自己的冻结文件，保留 `D:\Hy3` 的旧冻结、JSONL 和报告原样，不增加调用入口或冻结参数。新结果分别使用副本内的 `reports/calibrate_results_s7_contract01_20260907_e2a86459.jsonl`、`reports/final_results_s7_contract01_20260907_e2a86459.jsonl`、`reports/stability_results_s7_contract01_20260907_e2a86459.jsonl`。这些名称仅为方案，尚未创建或执行；执行前检查输出及对应锁、pending 的实际状态，不在新路径中填入历史行。方法方案批准不包含提交、创建副本、冻结、Live 费用或真实报告写入授权。

**已批准原子任务：文档级表达审计精简契约回填（2026-09-08）。** 用户在契约草案后要求避免过度工程化和防御性编程，并在精简方案后明确授权。唯一目标是将下列方法契约及实施边界同步至 `docs/DEV_PLAN.md`、`docs/paperlens_project_proposal.md`，仅允许修改这两个文件。输入为已批准的精简方案，输出为一致的文档；本任务不改变运行时输入输出、错误码或副作用。验证命令为 `git diff -- docs/DEV_PLAN.md docs/paperlens_project_proposal.md`、`git diff --check`、`git diff --name-only`、`git diff --cached --name-only`，并使用项目 Python 只读核对前后保护哈希。纯文档回填不要求业务红测，不运行测试、Live 或真实 MinerU，不暂存、提交、生成报告或重新冻结。

**状态与依据。** 本次回填基线为 `master / b50f0b7429e2b787ed6ce25a8f3912a18b634c9f`；当前实现仍为 `audit-v6`、`deep-audit-result-v2`，下列方法已批准但尚未实施。四组长度填充／术语堆砌攻击的完整文档均进入请求，但变更句没有目标 claim，攻击与干净配对的已验证语义配对相同。现有术语分只统计配对的术语判断；读者适配的句子拆分检查不能直接判断冗余或无解释术语。这是补足文档级表达信号的依据，不授权按历史成绩改判，也不把离线输入核验当作模型效果证据。

| 契约项 | 已批准的最小边界 |
|---|---|
| 检查范围与调用 | 检查完整正文，包括没有 claim 的句子；复用现有单批深审请求，不新增模式、调用入口或额外供应商调用。事实配对仍只依据自身 claim/evidence，不受其他句子的表达问题影响；原三类风险检查保留。 |
| 两类表达问题 | ①无信息增量的重复或偏离主题；②影响目标读者理解、且上下文未解释的术语。必要复述、合法长句和已充分解释的专业术语，不得仅因长度或关键词被判为问题。 |
| 最小返回信号 | 新版本深审响应独立表达两类检查的“未发现／发现／无法判断”状态；发现时返回对应类型、关联句子和必要的短摘录。未发现不能由字段缺失推定。不返回完整覆盖句子 ID 清单，不建立覆盖追踪框架；具体模型字段由后续实施任务以 `models.py` 为唯一契约来源定义。 |
| 必要校验与失败 | 复用现有严格 Schema、位置与摘录校验，检查类别及状态合法、必需内容存在、句子可定位、摘录与原句相符。位置合法不证明模型判断正确。“无法判断”不算通过或攻击命中，受影响的正式指标不可用；格式错误沿用既有校验、有限重试和失败路径，不新增重试层，不兜底成功或回退 Mock。 |
| 评分归属 | 两类表达问题归入读者适配；术语含义错误继续归术语准确性。读者适配保留五项检查及原分档：以新信号补足术语解释、表达组织两项，原有检查继续作为约束；同类多条问题不重复扣同一项。其余维度、八维权重及数值门槛不变，分数仍由代码计算。 |
| 攻击归属 | 对没有目标 claim 的 `length_padding`、`terminology_stuffing`，以与目标句关联的对应表达问题作为检出信号，替代 C 中这两类的整体维度回退。不得仅因整体读者适配低于 4 或 `ACADEMIC_INTEGRITY` 硬失败而认定对应攻击命中。目标确定性／语义信号、非目标 major/critical 告警，以及 `limitation_deletion`、`rubric_prompt_injection` 的原维度回退不变。攻击与干净配对使用同一规则，身份、质量标签、攻击标签和旧结果不得进入模型输入或成为特判依据。 |
| 版本与证据 | 这是评分实现和两类攻击归属的方法变更，须升级并记录 Prompt、响应 Schema 和评测方法版本。旧 v2 记录按旧契约读取，不补造新信号、不冒充新版本完整结果；原 final、独立失败复跑记录、报告及冻结均保留。未来正式证据的适用性、冻结及费用分别授权，不把已使用保留集称为全新盲测。 |

**生效与实施边界。** 上述契约仅在后续源码实现、离线验收和版本衔接完成后适用于新版本；既有 v2 及历史批次仍按原 A—F 解释。它明确修订 C 的两个分支，并扩展深审模型输入要求和响应信号；其余 A—F 原则保留，不能继续把原 D 的“不改变模型输入”误用于禁止这次已批准的扩展，也不能借此泄漏评测标签。不新增服务、目录、依赖、API、数据库表或通用框架；不顺带处理报告配对计数 P2、修订或其他已关闭事项。源码修改文件及版本兼容落点由后续实施任务明确，本次两文件授权不是源码实施授权；超过任务白名单或需要额外契约变更时停止报告。

**后续离线验收。** 先补真实业务断言失败，再做最小实现；通过注入 FakeClient 捕获实际发送载荷，检查完整正文、原配对独立性、新信号要求、风险检查、无标签泄漏及单批调用。计分和归属测试覆盖两类问题及必要复述、已解释术语、合法长句、无关位置问题、无法判断、无 claim／risk-only 和旧版本读取；重点验证受影响的业务边界，不机械穷举或弱化既有测试。按实施任务执行定向及必要回归；离线通过不宣称真实模型效果改善，不触发自动 Live 或重跑旧结果。

本回填任务保留前序工作及全部真实产物，完成后停在 `READY_FOR_REVIEW`；`STAGE_7=HOLD`、`STAGE_8_ENTRY=HOLD`、`PRODUCTION_READY=NO`。
