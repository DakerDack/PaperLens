...
文档级表达审计的精简扩展已于 2026-09-08 批准，尚未实施，见第 7.7 节。它补足术语解释和表达组织的信号，保留五项检查及本表分档；术语含义错误与表达堆砌分开，不因整体风险拒绝而重复扣表达分。
...
上述 A—F 描述既有版本。第 7.7 节已批准的表达审计扩展将在新版本实施并验收后，替代 C 中长度填充、术语堆砌的两个回退分支；本次文档批准不改判旧结果，也不表示新规则已经运行。
...
### 7.7 已批准的文档级表达审计精简契约（2026-09-08，尚未实施）

用户要求避免过度工程化和防御性编程，并批准精简方案及两文件回填。本任务仅修改 `docs/DEV_PLAN.md`、`docs/paperlens_project_proposal.md`，不改变源码、运行时错误码、材料或真实产物。当前源码基线为 `b50f0b7429e2b787ed6ce25a8f3912a18b634c9f`，仍使用 `audit-v6`、`deep-audit-result-v2`。第 7.6 节“不新增 Schema 字段”等描述属于此前方案；本节允许未来版本增加下述最小表达信号，但不授权本轮实施。

**目的与检查范围。** 现有非事实表达能进入完整文档风险审计，却不一定进入已验证语义配对；原术语与读者适配评分不能直接区分本次长度填充、术语堆砌样例。扩展在同一深审请求中检查完整正文，包括没有 claim 的句子，只增加两类表达问题：无信息增量的重复或偏离主题；影响目标读者理解且上下文未解释的术语。必要复述、合法长句、充分解释的专业术语不得仅因长度或关键词被扣分。事实配对仍只依据自身证据，原三类风险检查继续保留，不新增供应商调用、模式或入口。

**最小信号与校验。** 两类检查分别明确返回“未发现／发现／无法判断”；发现时给出类型、关联句子和必要的短摘录，不能把缺失当作未发现。不要求完整覆盖句子 ID 清单，不建立覆盖追踪框架。后续实施以 `models.py` 定义具体契约，复用现有严格 Schema、位置及摘录校验，拒绝不合法或缺少必需内容的响应；位置可核验不等于语义判断正确。“无法判断”不作为通过或命中，受影响的正式指标不可用。格式错误沿用既有校验、有限重试与失败路径，不增加兜底成功、额外重试层或 Mock 回退。

**评分与类别归属。** 两类表达问题归入读者适配，术语含义错误继续归术语准确性。代码用新信号补足五项检查中的术语解释、表达组织两项，原检查继续作为约束，同类多条问题不重复扣同一项；模型不得返回分数。五项分档、八维权重及其他维度不变。没有目标 claim 时，`length_padding`、`terminology_stuffing` 以与目标句关联的对应表达问题检出，替代原 C 中这两类的整体维度回退；不能直接把整体读者适配低于 4 或 `ACADEMIC_INTEGRITY` 硬失败计成对应攻击命中。目标确定性／语义信号、非目标 major/critical 保守告警，以及限制删除、rubric 提示注入的原维度回退不变。攻击与干净配对遵守同一规则，不把身份、质量标签、攻击标签或历史结果泄漏进模型输入或用于特判。

**方法变更与兼容。** 虽然 13/16、1/16 等数值目标及固定样本规模不变，评分实现和两类攻击归属已经构成方法变更，未来须升级并记录 Prompt、响应 Schema 及评测方法版本。扩展仅在源码实现、离线验收及版本衔接完成后对新版本生效；旧 v2 按旧契约读取，不填充或伪造新信号，不将旧结果视为新版本的完整验证。既有 final、独立失败复跑记录、报告、冻结和数据库均原样保留；新正式实验的证据适用性、冻结和费用分别授权，已使用保留集不重新称为全新盲测。

**最小实施与离线验收边界。** 不新增服务、目录、依赖、API、数据库表或通用框架，不顺带修复其他台账项。后续源码任务先明确白名单及兼容落点，先业务红测、最小实现，再定向和必要回归。通过 FakeClient 验证实际请求的完整文档、独立配对、新信号、风险检查、无标签泄漏和单批调用；业务正反例覆盖两类问题、必要复述、已解释术语、合法长句、无关位置问题、无法判断、无 claim／risk-only 及旧版本读取，不机械穷举或弱化测试。离线通过只证明输入和确定性处理正确，不证明真实模型效果改善。

本次文档验证仅为两文件差异、口径一致性、`git diff --check`、白名单与暂存区检查及前后保护哈希；不运行测试、Live、真实 MinerU，不提交、重新冻结或生成报告。具体原子任务卡见 `DEV_PLAN.md` 阶段 7。完成后为 `READY_FOR_REVIEW`；`STAGE_7=HOLD`、`STAGE_8_ENTRY=HOLD`、`PRODUCTION_READY=NO`。
